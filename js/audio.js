...
// 存音乐详情
var musicData = [['Lose My Mind', '岑海峻25216950224'], ['F1-Hans Zimmer', '岑海峻25216950224'], ['The Business-Tiësto', '岑海峻25216950224'], ['Messy-ROSÉ', '岑海峻25216950224']]
var musicId = 0;
function initMusic() {
...
// 音量控制
var lastVolum = 70;
audio.volumn = lastVolum / 100;
volumnTogger.addEventListener('input', updateVolume);
function updateVolume() {
    var volumnValue = volumnTogger.value / 100;
    audio.volumn = volumnValue;
    if (volumnValue > 0) {
        audio.muted = false;
    }
    updateVolumeIcon();
...
// 静音按钮控制
volumn.addEventListener('click', setNoVolume)
function setNoVolume() {
    if (audio.muted || audio.volumn === 0) {
        audio.muted = false;
        volumnTogger.value = lastVolum / 100;
    } else {
        audio.muted = true;
        lastVolum = volumnTogger.value;
